            ...
            steps {
                dir('.metrics/jenkins') {
                    withEnv(["FIREBASE_PROJECT_ID=${params.FIREBASE_PROJECT_ID}", "METRICS_PROJECT_ID=${params.METRICS_PROJECT_ID}", "ANCESTOR_JOB_NAME=${params.ANCESTOR_JOB_NAME}", "CI_INTEGRATION_JENKINS_URL=${CI_INTEGRATION_JENKINS_URL}"]) {
                        withCredentials([usernamePassword(credentialsId: 'prod_app_credentials', passwordVariable: 'WEB_APP_USER_PASSWORD', usernameVariable: 'WEB_APP_USER_EMAIL')]) {
                            withCredentials([string(credentialsId: 'prod_ci_integrations_firebase_api_key', variable: 'CI_INTEGRATIONS_FIREBASE_API_KEY')]) {
